...
export const registro = async (req, res) => {  // req es la solicitud y res es la respuesta
  try {
    const { usuario, email, password } = req.body; // extrae los datos del cuerpo de la solicitud
    const user = new Usuarios({ usuario, email, password }); // crea un nuevo usuario con los datos recibidos
        
    await user.save(); // guarda el usuario en la base de datos
    
    const token = jwt.sign({ userId: user._id }, process.env.JWT_SECRET, { expiresIn: '1h' });// genera un token JWT con el id del usuario y lo expira en 1 hora
